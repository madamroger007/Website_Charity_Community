...
//*******************************************  Get function
function get_project(topic) {
    $.ajax({
        type: 'GET',
        url: "/get_project",
        success: function (response) {
            // Perbarui tabel dengan data pengguna baru
            const memo = topic == "" ? response.projects : response.projects.filter(article => article.topic === topic);
            

            console.log(memo)
            getComponentTopicProject(memo)
...
function showUpdateCard(newsId) {
    $.ajax({
        ...
        success: function (response) {
            ...
            ShowUpdateComponent(response.projects)
...
function update_project(newsId) {
    let formData = new FormData($("#updateForm")[0]);
  
    $.ajax({
        type: "POST",
        url: "/update_project/" + newsId,
        headers: {
            'X-CSRFToken': csrfToken
        },
        data: formData,
...
function delete_project(newsId) {
    console.log(newsId)
    $.ajax({
        type: 'POST',
        url: "/delete_project/" + newsId,
        headers: {
...
<header class="modal-card-head">
<p class="modal-card-title">Posting Project</p>
...
<div class="field">
<label for="input-pic" >Gambar</label>
<div class="control is-expanded">
<div class="file has-name">
<label class="file-label" style="width: 100%;">
...
<footer class="modal-card-foot">
<button type="button" class="button is-success" onclick="submitProject()">Posting</button>
...
function getComponentTopicProject(news) {
